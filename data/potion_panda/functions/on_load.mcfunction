team add panda
team modify panda color blue
team modify panda friendlyFire false
team modify panda prefix "Potion Panda - "

scoreboard objectives add panda_tool trigger
scoreboard objectives add panda_item trigger
scoreboard objectives add panda_banner trigger
scoreboard objectives add panda_bannerhead trigger
scoreboard objectives add panda_shield trigger
scoreboard objectives add panda_equip trigger
scoreboard objectives add panda_giveme trigger
scoreboard objectives add panda_giveyou trigger
scoreboard objectives add panda_clear trigger
scoreboard objectives add panda_particle trigger
scoreboard objectives add panda_pet trigger
scoreboard objectives add panda_offhand dummy

scoreboard objectives add panda_block trigger
scoreboard objectives add panda_stand dummy
scoreboard objectives add panda_shift custom:sneak_time

scoreboard objectives add panda_cannon_get trigger
scoreboard objectives add panda_cannon_mod trigger

scoreboard objectives add panda_giveme_set dummy
scoreboard objectives add panda_giveyo_set dummy
scoreboard objectives add panda_clear_set dummy

scoreboard objectives add panda_cookie used:cookie
