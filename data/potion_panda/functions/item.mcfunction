...
execute as @a[tag=panda,scores={panda_item=2..}] run scoreboard players set @s panda_item 0

execute as @a[tag=panda,scores={panda_item=1}] unless entity @s[nbt={Inventory:[{"id":"minecraft:cookie",Count:2b}]}] run item replace entity @s weapon.offhand with minecraft:cookie{CustomModelData:1,display:{Name:'{"text":"Bamboo Panda Cake"}'}} 2
